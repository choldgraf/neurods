...
def unmask(data, mask, bg_value=0):
    ...
    if np.ndim(data)==1:
        ...
        br = np.ones((data.shape[0],)+mask.shape)
        br[:, mask] = data
    else:
...
def hrf(shape='twogamma', sf=1, pttp=5, nttp=15, pnr=6, ons=0, pdsp=1, ndsp=1, t=None):
    ...
    sf : scalar
        HRF sample frequency (TR), in seconds (default = 2)
    pttp : scalar
